...
extern MY_HIGHP_OR_MEDIUMP vec2 faded_holo;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    vec4 hsl = HSL(0.8*tex + 0.2*vec4(0.,0.,1.,tex.a));

	float t = faded_holo.y*7.221 + time;
	...
	float res = (.5 + .5* cos( (faded_holo.x) * 2.612 + ( field + -.5 ) *3.14));

...
extern PRECISION vec2 ectoplasmic;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float f =  clamp((fbm(st+r) + 1.0) / 2.0, 0, 1);
    float g = clamp(length(r.x),0.0,1.0);
    float h = clamp(length(r.y),0.0,1.0);
